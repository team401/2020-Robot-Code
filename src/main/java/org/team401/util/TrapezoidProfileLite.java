...
/**
 * Trapezoid profile that doesn't phone home.
 * 
 ...
 *   new TrapezoidProfileLite.Constraints(kMaxV, kMaxA);
 * TrapezoidProfileLite.State previousProfiledReference =
 *   new TrapezoidProfileLite.State(initialReference, 0.0);
 * </code></pre>
 *
 * <p>Run on update:
 * <pre><code>
 ...
 * `isFinished()`.
 */
public class TrapezoidProfileLite {
    // The direction of the profile, either 1 for forwards or -1 for inverted
    private int m_direction;

    private TrapezoidProfileLite.Constraints m_constraints;
    private TrapezoidProfileLite.State m_initial;
    private TrapezoidProfileLite.State m_goal;
    ...
    public static class Constraints {
        ...
        /**
         * Construct constraints for a TrapezoidProfileLite.
         *
    ...
    public static class State {
        ...
        public boolean equals(Object other) {
            if (other instanceof TrapezoidProfileLite.State) {
                TrapezoidProfileLite.State rhs = (TrapezoidProfileLite.State) other;
    ...
    /**
     * Construct a TrapezoidProfileLite.
     *
     * @param constraints The constraints on the profile, like maximum velocity.
     * @param goal        The desired state when the profile is complete.
     * @param initial     The initial state (usually the current state).
     */
    public TrapezoidProfileLite(TrapezoidProfileLite.Constraints constraints, TrapezoidProfileLite.State goal, TrapezoidProfileLite.State initial) {
     ...
     * @param goal        The desired state when the profile is complete.
     */
    public TrapezoidProfileLite(TrapezoidProfileLite.Constraints constraints, TrapezoidProfileLite.State goal) {
        this(constraints, goal, new TrapezoidProfileLite.State(0, 0));
    }
     ...
     */
    public TrapezoidProfileLite.State calculate(double t) {
        TrapezoidProfileLite.State result = new TrapezoidProfileLite.State(m_initial.position, m_initial.velocity);
     ...
     * @param goal        The desired state when the profile is complete.
     */
    private static boolean shouldFlipAcceleration(TrapezoidProfileLite.State initial, TrapezoidProfileLite.State goal) {
        return initial.position > goal.position;
    }

    // Flip the sign of the velocity and position if the profile is inverted
    private TrapezoidProfileLite.State direct(TrapezoidProfileLite.State in) {
        TrapezoidProfileLite.State result = new TrapezoidProfileLite.State(in.position, in.velocity);
